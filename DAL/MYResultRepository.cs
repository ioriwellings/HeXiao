...
namespace Langben.DAL
{
    ...
    /// 对比结果
    /// </summary>
    public partial class MYResultRepository : BaseRepository<MYResult>, IDisposable
    {
        /// <summary>
        /// 查询的数据
        /// </summary>
        ...
        /// <returns></returns>      
        public IQueryable<MYResult> GetData(SysEntities db, string order, string sort, string search, params object[] listQuery)
        {
            ...
            return ((System.Data.Entity.Infrastructure.IObjectContextAdapter)db).ObjectContext 
                     .CreateObjectSet<MYResult>().Where(string.IsNullOrEmpty(where) ? "true" : where)
                     .OrderBy("it.[" + sort.GetString() + "] " + order.GetString())
                     .AsQueryable(); 

        }
        /// <summary>
        /// 通过主键id，获取对比结果---查看详细，首次编辑
        /// </summary>
        ...
        /// <param name="id">主键</param>
        /// <returns>对比结果</returns>
        public MYResult GetById(SysEntities db, string id)
        { 
            return db.MYResult.SingleOrDefault(s => s.Id == id);
        ...
        /// <param name="id">一条对比结果的主键</param>
        public void Delete(SysEntities db, string id)
        {
            MYResult deleteItem = GetById(db, id);
            if (deleteItem != null)
            { 
                db.MYResult.Remove(deleteItem);
            }
        }
        /// <summary>
        /// 删除对象集合
        ...
        {
            //数据库设置级联关系，自动删除子表的内容   
            IQueryable<MYResult> collection = from f in db.MYResult
