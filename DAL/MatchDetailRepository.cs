...
namespace Langben.DAL
{
    ...
    /// 匹配项对比项设定
    /// </summary>
    public partial class MatchDetailRepository : BaseRepository<MatchDetail>, IDisposable
    {
        /// <summary>
        /// 查询的数据
        /// </summary>
        ...
        /// <returns></returns>      
        public IQueryable<MatchDetail> GetData(SysEntities db, string order, string sort, string search, params object[] listQuery)
        {
            ...
            return ((System.Data.Entity.Infrastructure.IObjectContextAdapter)db).ObjectContext 
                     .CreateObjectSet<MatchDetail>().Where(string.IsNullOrEmpty(where) ? "true" : where)
                     .OrderBy("it.[" + sort.GetString() + "] " + order.GetString())
                     .AsQueryable(); 

        }
        /// <summary>
        /// 通过主键id，获取匹配项对比项设定---查看详细，首次编辑
        /// </summary>
        /// <param name="id">主键</param>
        /// <returns>匹配项对比项设定</returns>
        ...
        /// <param name="id">主键</param>
        /// <returns>匹配项对比项设定</returns>
        public MatchDetail GetById(SysEntities db, string id)
        { 
            return db.MatchDetail.SingleOrDefault(s => s.Id == id);
        ...
        /// <param name="id">一条匹配项对比项设定的主键</param>
        public void Delete(SysEntities db, string id)
        {
            MatchDetail deleteItem = GetById(db, id);
            if (deleteItem != null)
            { 
                db.MatchDetail.Remove(deleteItem);
            }
        }
        /// <summary>
        /// 删除对象集合
        ...
        {
            //数据库设置级联关系，自动删除子表的内容   
            IQueryable<MatchDetail> collection = from f in db.MatchDetail
        ...
        /// <summary>
        /// 根据RuleId，获取所有匹配项对比项设定数据
        /// </summary>
        /// <param name="id">外键的主键</param>
        /// <returns></returns>
        public IQueryable<MatchDetail> GetByRefRuleId(SysEntities db, string id)
        {
            return from c in db.MatchDetail
                        where c.RuleId == id
                        select c;
