...
namespace Common
{
    public class UploadFiles
    {
        ...
        public string fileSaveAs2(HttpPostedFileBase postedFile, string fileName)
        {
        ...
        public string fileSaveAsResult(HttpPostedFileBase postedFile)
        {
            string fileName = postedFile.FileName;//文件名
            string ramName = fileName.Substring(0, fileName.LastIndexOf('.'));
            string fileExt = GetPostfixStr(fileName); //文件扩展名，不含“.”
            string ramFileName = Common.Result.GetNewId() + "." + fileExt; //随机文件名
            string dirPath = "/up/Match/" + DateTime.Now.ToString("yyyyMM") + "/"; //上传目录相对路径
            string serverFileName = dirPath + ramFileName;
            //物理完整路径                    
            string toFileFullPath = GetMapPath(dirPath);
            //检查有该路径是否就创建
